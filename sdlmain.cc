...
int main(void)
{
	...
	Texture diffuse("p9_24.tga");
	//Texture diffuse("african_head_diffuse.tga");

        unsigned int curTime = SDL_GetTicks();
	unsigned int lastTime = curTime;
	for (int i = 0; i < fb.width; i++) {
		for (int j = 0; j < fb.height; j++) {
			fb.pixel(i, j, diffuse.get(i, j, fb.fmt));
		}
	}
	fb.update();
	...
	for (p.x = bboxmin.x; p.x <= bboxmax.x; p.x++) {
		for (p.y = bboxmin.y; p.y <= bboxmax.y; p.y++) {
			...
			if (z > zb.get(p.x, p.y)) {
				...
				fb.pixel(p.x, p.y, diffuse.get(uv[0], uv[1], fb.fmt));
			}
		}
	}
}
